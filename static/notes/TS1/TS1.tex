...
There are different ways to model stationary time series. Here, we will explore the properties of th following three models.
...
Using the independence between \(\epsilon_t\) and \(Y_{t-1}\), \(E(\epsilon_t Y_{t-k})=0\) and the results in Eq. 3 (This valid for AR(2)) we have
...
\textbf{Question: } Similar to the results in Equation \eqref{eq:yule2} for \(AR(2)\) process, obtain the following recursive relationship for \(AR(p)\).
...
The parameters can be extimated using

\[\bm{\phi}=\bm{P_p^{-1}\rho_p}.\]

\textbf{Question:} Obtain the parameters of an \(AR(3)\) process whose first autocorrelations are \(\rho_1=0.9\); \(\rho_2=0.9\); \(\rho_3=0.5\). Is the process stationary?
...
\textbf{Question:} For \(AR(2)\) process show that \(\phi_{kk}=0\) for all \(k>2\). Sketch the PACF of \(AR(2)\) process.

\hypertarget{pacf-for-arp-model}{%
\subsubsection{PACF for AR(P) model}\label{pacf-for-arp-model}}

In general for \(AR(p)\) precess, the partial autocorrelation function \(\phi_{kk}\) is non-zero for \(k\) less than or equal to \(p\) (the order of the process) and zero for all \(k\) greater than \(p\). In other words, the partial autocorrelation function of a \(AR(p)\) process has a cutoff after lag \(p\).
...
whre \(\theta_0\) is a constant and \({\epsilon_t}\) is a white noise series.

\hypertarget{mean-3}{%
\subsubsection{Mean}\label{mean-3}}

\textbf{Question:} Show that \(E(Y_t) = \theta_0\).

\hypertarget{variance-2}{%
\subsubsection{Variance}\label{variance-2}}

\textbf{Question:} Show that \(Var(Y_t) = (1+\theta_1^2)\sigma^2\).

We can see both mean and variance are time-invarient. \(MA\) models are finite linear combinations of a white noise sequence. Hence, \(MA\) processes are always weakly stationary.
...
To obtain the autocorrelation function of \(MA(1)\), we first multiply bothsides of Equation \eqref{eq:ma2} by \(Y_{t-k}\) and take the expectation.
...
We have \(\gamma_0=\sigma^2(1+\theta_1^2)\).
...
\textbf{Question: } Show that \(Var(Y_t) = \sigma^2 (1+\theta_1^2 + \theta_2^2).\)
...
\textbf{Question:} Show that the constant term of an \(MA\) model is the mean of the series (i.e. \(E(Y_t)=\theta_0\)).

\hypertarget{variance-4}{%
\subsubsection{Variance}\label{variance-4}}

\textbf{Question:} Show that the varianceof an \(MA\) model is
...
\textbf{Question:} Show that the autocorrelation function of a \(MA(q)\) process is zero, beyond the order of \(q\) of the process. In other words, the autocorrelation function of a moving average process has a cutoff after lag \(q\).
...
\textbf{Next, we conside the realation MA(q) \textless{}--\textgreater{} AR(}\(\infty\)\textbf{)}
